...
///     <name> <sig>
///   Where <sig> is the bind: format (e.g., "iii:v", "d:d", ":v")
///
...
///   : _lib ;
///   : InitWindow _lib "InitWindow" dl-sym bind: iii:v ;
///   ...
...
pub fn main() !void {
    ...
    if (args.len < 2) {
        ...
        try stderr.print("  s = temp string        S = persistent string\n", .{});
        std.process.exit(1);
    }
    ...
    try w.print(":: _lib \"{s}\" dl-open ;\n", .{library_path.?});

    // Emit constant fptr for each function, then a thin wrapper with bind:
    for (functions.items) |func| {
        try w.print(":: _{s} _lib \"{s}\" dl-sym ;\n", .{ func.name, func.name });
    }
    try w.print("\n", .{});
    for (functions.items) |func| {
        try w.print(": {s}  _{s} bind: {s} ;\n", .{ func.name, func.name, func.sig });
    }
